...
@section('titulo', 'Editar Membros')

@section('conteudo')

    <h3 class="p-4 pb-0" style="color:#312f2f">Editar Membro</h3>

    <form action="{{ route('admin.membro.update',['id'=>$user->id]) }}" method="post" enctype="multipart/form-data" class="mt-3 mb-5">
        @csrf
        <div class="card-body">
    ...
    @if (session('membro.update.success'))
            <script>
                Swal.fire(
    ...
    @if (session('membro.update.error'))
            <script>
                Swal.fire(
